...
BETA_REDUCE_PUSH_RESULT:
  ...
  BTORLOG ("%s: result %s (%d)", __FUNCTION__, node2string (result),
                ...
                if (BTOR_IS_FUN_NODE (BTOR_REAL_ADDR_NODE (e[1])))
                  {
                    ...
                    btor_release_exp (btor, e[1]);
                    if (to_prop
                        && BTOR_IS_APPLY_NODE (BTOR_REAL_ADDR_NODE (result)))
                      {
                        if (!btor_find_in_ptr_hash_table (to_prop,
                                BTOR_REAL_ADDR_NODE (result)))
                          btor_insert_in_ptr_hash_table (to_prop,
                                btor_copy_exp (btor,
                                    BTOR_REAL_ADDR_NODE (result)));
                      }
                  }
                        ...
                        if (tmp)
                          {
                            ...
                            if (!btor_find_in_ptr_hash_table (tmp, t0))
                              btor_insert_in_ptr_hash_table (tmp,
                                  t0)->data.asPtr = e[0];
                            else
...
btor_beta_reduce_partial (Btor * btor, BtorNode * exp, int * evalerr,
                          BtorPtrHashTable * to_prop, BtorPtrHashTable * conds)
{
  BTORLOG ("%s: %s", __FUNCTION__, node2string (exp));
