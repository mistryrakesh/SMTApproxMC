      ...
      if (!btor_find_in_ptr_hash_table (visited, real_exp))
        {
          ...
          if (zero_extend)
            {
              fmt = sdc->version == 1 ? " (zero_extend[%d] "
                                      : " ((_ zero_extend %d) ";
...
static void
dump_smt_aux (Btor * btor, FILE * file, int version, BtorNode ** roots,
              int nroots)
{
  assert (btor);
  assert (file);
...
btor_dump_smt2_node (Btor * btor, FILE * file, BtorNode * exp, unsigned depth)
{
  ...
  BTOR_INIT_STACK (all);
  BTOR_INIT_STACK (visit);
  sdc = new_smt_dump_context (btor, file, 2);
