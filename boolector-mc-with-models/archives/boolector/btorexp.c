...
int
btor_precond_apply_exp_dbg (const Btor * btor, const BtorNode * fun,
                            const BtorNode * args)
{
  assert (btor);
  assert (fun);
...
inc_exp_ref_counter (Btor * btor, BtorNode * exp)
{
  ...
  real_exp = BTOR_REAL_ADDR_NODE (exp);
  BTOR_ABORT_NODE (
      real_exp->refs == INT_MAX, "Node reference counter overflow");
  real_exp->refs++;
}
  ...
  while (!BTOR_EMPTY_STACK (visit))
    {
      ...
      if (btor_find_in_ptr_hash_table (marked, real_cur))
        continue;
  ...
  switch (exp->kind)
    {
      ...
      case BTOR_UF_NODE:
        btor_release_sort (&btor->sorts_unique_table,
                           ((BtorUFNode *) exp)->sort);
...
#ifndef NDEBUG
static int is_valid_kind (BtorNodeKind kind)
{
...
really_deallocate_exp (Btor * btor, BtorNode * exp)
{
  ...
  if (exp->invbits)
    btor_freestr (btor->mm, exp->invbits);
...
connect_child_exp (Btor * btor, BtorNode * parent, BtorNode * child, int pos)
{
  ...
  if (BTOR_REAL_ADDR_NODE (child)->apply_below)
    parent->apply_below = 1;
...
static BtorNode **
find_lambda_exp (Btor * btor, BtorNode * param, BtorNode * body,
                 unsigned int * lambda_hash, int compare_lambdas)
{
  assert (btor);
  assert (param);
  assert (body);
...
static int
compare_lambda_exp (Btor * btor, BtorNode * param, BtorNode * body,
                    BtorNode * lambda)
{
  assert (btor);
  assert (param);
  ...
  if (BTOR_IS_LAMBDA_NODE (BTOR_REAL_ADDR_NODE (body))
      && BTOR_IS_LAMBDA_NODE (BTOR_REAL_ADDR_NODE (lambda->e[1])))
    {
      ...
      while (has_next_lambda_iterator (&it))
        {
          if (!has_next_lambda_iterator (&iit)) 
          ...
          btor_insert_in_ptr_hash_table (
            param_map, param)->data.asPtr = subst_param;
          //printf ("%s -> %s\n", node2string (param), node2string (subst_param));
        }
    }
...
//            printf ("  e[%d]: %s\n", i, node2string (e[i]));
          ...
          if (BTOR_IS_SLICE_NODE (real_cur))
            {
              result =
                find_slice_exp (btor, e[0], real_cur->upper, real_cur->lower);
            }
...
//          printf ("visit (2): %s\n", node2string (cur));
          assert (b->data.asPtr);
          BTOR_PUSH_STACK (mm, args,
                           BTOR_COND_INVERT_NODE (cur, b->data.asPtr));
//          printf ("  result: %s\n", node2string (b->data.asPtr));
  ...
  if (!BTOR_EMPTY_STACK (args))
    equal = BTOR_TOP_STACK (args) == lambda->e[1];
...
btor_get_symbol_exp (Btor * btor, BtorNode * exp)
{
  ...
  assert (exp);
  assert (btor == BTOR_REAL_ADDR_NODE (exp)->btor);
  BtorPtrHashBucket *b = btor_find_in_ptr_hash_table (btor->node2symbol,
