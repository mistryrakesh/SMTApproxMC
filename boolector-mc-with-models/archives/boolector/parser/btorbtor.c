...
parse_var (BtorBTORParser * parser, int len)
{
  ...
  res = boolector_var (parser->btor, len, 
...
parse_param (BtorBTORParser * parser, int len)
{
  ...
  res = boolector_param (parser->btor, len, 
            parser->symbol.start[0] ? parser->symbol.start : 0);
...
parse_array (BtorBTORParser * parser, int len)
{
  ...
  res = boolector_array (parser->btor, len, idx_len, 
            parser->symbol.start[0] ? parser->symbol.start : 0);
  boolector_set_btor_id (parser->btor, res, parser->idx);
  BTOR_PUSH_STACK (parser->mem, parser->inputs, res);
...
parse_fun_exp (BtorBTORParser * parser, int len)
{
  ...
  if (boolector_is_fun (parser->btor, res))
    return res;
...
parse_root (BtorBTORParser * parser, int len)
{
  ...
  if (!(res = parse_exp (parser, len, 0, 1)))
    return 0;
...
parse_unary (BtorBTORParser * parser, int len, Unary f)
{
  ...
  if (!(tmp = parse_exp (parser, len, 0, 1)))
    return 0;
                            ...
                            int len, Binary f, int can_be_array)
{
  ...
  if (!(l = parse_exp (parser, 0, can_be_array, 1)))
    return 0;
...
parse_shift (BtorBTORParser * parser, int len, Shift f)
{
  ...
  if (!(r = parse_exp (parser, rlen, 0, 1)))
    goto RELEASE_L_AND_RETURN_ERROR;
...
parse_lambda (BtorBTORParser * parser, int len)
{
  ...
  if (!(params[0] = parse_param_exp (parser, paramlen)))
    return 0;
...
parse_apply (BtorBTORParser * parser, int len)
{
  ...
  if (!(fun = parse_fun_exp (parser, len)))
    return 0;
...
btor_parse_btor_parser (BtorBTORParser * parser, BtorCharStack * prefix,
                        FILE * infile, const char *infile_name,
                        FILE * outfile, BtorParseResult * res)
{
  ...
  BTOR_MSG (boolector_get_btor_msg (parser->btor), 1, "parsing %s",
            infile_name);
      ...
      if (res)
        {
          ...
          BTOR_MSG (boolector_get_btor_msg (parser->btor), 1, 
              "parsed %d inputs", res->ninputs);
          BTOR_MSG (boolector_get_btor_msg (parser->btor), 1, 
              "parsed %d outputs", res->noutputs);
        }
