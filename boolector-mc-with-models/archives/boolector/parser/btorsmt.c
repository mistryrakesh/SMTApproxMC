...
btor_delete_smt_node (BtorSMTParser * parser, BtorSMTNode * node)
{
  ...
  if (node->exp)
    boolector_release (parser->btor, node->exp);
...
push_input (BtorSMTParser * parser, BoolectorNode * v)
{
  if (parser->model)
    BTOR_PUSH_STACK (parser->mem,
                     parser->inputs, boolector_copy (parser->btor, v));
}
...
INVALID_SORT:
  ...
  if (!strcmp (p, "Bool"))
  ...
  else if (has_prefix (p, "Array"))
    {
      ...
      symbol->exp = boolector_array (parser->btor, datalen, addrlen,
                                    symbol->name);
...
translate_binary (BtorSMTParser * parser,
                  BtorSMTNode * node, const char * name,
                  BoolectorNode * (*f)(Btor *, BoolectorNode *, BoolectorNode *))
{
  BtorSMTNode * c0, * c1;
  BoolectorNode * a0, * a1;
...
translate_extract (BtorSMTParser * parser, BtorSMTNode * node)
{
  ...
  translate_node (parser, node,
                  boolector_slice (parser->btor, exp, upper, lower));
}
  ...
  if (len == 1)
    {
      ...
      if (f == boolector_sra)
      ...
      else
        {
          ...
          translate_node (parser, node,
                          boolector_and (parser->btor, a0, tmp));
          ...
          switch (symbol->token)
            {
              ...
              case BTOR_SMTOK_AND:
                translate_associative_binary (parser,
                                              node, "and", boolector_and);
                break;
              case BTOR_SMTOK_OR:
                translate_associative_binary (parser,
                                              node, "or", boolector_or);
                break;
              ...
              case BTOR_SMTOK_XOR:
                translate_associative_binary (parser,
                                              node, "xor", boolector_xor);
                break;
              case BTOR_SMTOK_IFF:
                translate_associative_binary (parser,
                                              node, "iff", boolector_xnor);
                break;
                                     ...
                                     BoolectorNode * exp)
{
  ...
  if (satres == BOOLECTOR_SAT)
  ...
  else
    {
      ...
      if (res->result == BOOLECTOR_UNKNOWN)
        res->result = BOOLECTOR_UNSAT;
      fprintf (parser->outfile, "unsat\n");
    }
  if (parser->verbosity >= 2)
    boolector_print_stats (parser->btor);
...
parse (BtorSMTParser * parser, BtorCharStack * prefix,
       FILE * infile, const char * infile_name, FILE * outfile,
       BtorParseResult * res)
{
