...
delete_aig_node (BtorAIGMgr * amgr, BtorAIG * aig)
{
  ...
  if (BTOR_IS_CONST_AIG (aig))
    return;
  if (aig->cnf_id)
    btor_release_cnf_id_aig_mgr (amgr, aig);
...
static int
find_and_contradiction_aig (BtorAIGMgr *amgr,
                            BtorAIG *aig, BtorAIG *a0, BtorAIG *a1,
                            int *calls)
{
  assert (amgr);
  assert (aig);
  ...
  if (!BTOR_IS_INVERTED_AIG (aig) && BTOR_IS_AND_AIG (aig))
    {
      if (BTOR_LEFT_CHILD_AIG (aig) == BTOR_INVERT_AIG (a0) ||
        ...
        return 1;
      *calls += 1;
      return find_and_contradiction_aig (amgr,
               BTOR_LEFT_CHILD_AIG (aig), a0, a1, calls) ||
             find_and_contradiction_aig (amgr,
               BTOR_RIGHT_CHILD_AIG (aig), a0, a1, calls);
    }
  return 0;
}
...
static BtorAIG *
clone_aig (BtorMemMgr * mm, BtorAIG * aig, BtorAIGPtrPtrStack * children,
           BtorAIGMap * aig_map)
{
  assert (mm);
  assert (aig);
...
clone_aig_unique_table (BtorMemMgr * mm,
                        BtorAIGUniqueTable * table, BtorAIGUniqueTable * res,
                        BtorAIGMap * aig_map)
{
  assert (mm);
  assert (table);
...
clone_aig_id2aig (BtorMemMgr * mm,
                  BtorAIGPtrStack * id2aig, BtorAIGPtrStack * res,
                  BtorAIGMap * aig_map)
{
  assert (mm);
  assert (id2aig);
  ...
  while (!BTOR_EMPTY_STACK (stack))
    {
      ...
      if ((isxor = btor_is_xor_aig (amgr, root, &leafs))) isite = 0;
      else isite = btor_is_ite_aig (amgr, root, &leafs);
              ...
              for (p = leafs.start; p < leafs.top; p++)
                {
                  ...
                  btor_add_sat (smgr,
                                BTOR_GET_CNF_ID_AIG (BTOR_INVERT_AIG (left))); 
                }
              btor_add_sat (smgr, 0);
            }
          else
...
btor_cmp_aig (BtorAIG * a, BtorAIG * b) {
  if (a == b)
    return 0;
  if (BTOR_INVERT_AIG (a) == b)
    return BTOR_IS_INVERTED_AIG (a) ? -1 : 1;
  if (BTOR_IS_INVERTED_AIG (a))
    a = BTOR_INVERT_AIG (a);
  if (a == BTOR_AIG_FALSE)
    return -1;
  assert (a != BTOR_AIG_TRUE);
  if (BTOR_IS_INVERTED_AIG (b))
    b = BTOR_INVERT_AIG (b);
  if (b == BTOR_AIG_FALSE)
    return 1;
