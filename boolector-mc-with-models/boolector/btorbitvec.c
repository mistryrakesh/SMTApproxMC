...
btor_assignment_bv (BtorMemMgr * mm, BtorNode * exp, int init_x_values)
{
  ...
  if (!real_exp->av)
    return btor_new_bv (mm, real_exp->len);
...
static void
udiv_urem_bv (BtorMemMgr * mm, BtorBitVector * a, BtorBitVector * b,
              BtorBitVector ** q, BtorBitVector ** r)
{
  assert (mm);
  assert (a);
  assert (b);
