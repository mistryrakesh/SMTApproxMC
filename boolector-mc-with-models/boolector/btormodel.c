...
static void
find_candidates_for_param (Btor * btor, BtorNode * param,
      ...
      switch (parent->kind)
        {
          case BTOR_BEQ_NODE:
            ...
            BTOR_PUSH_STACK (mm, *candidates,
                             BTOR_INVERT_NODE (parent->e[pos]));
            break;
  ...
  for (i = 0; i < BTOR_COUNT_STACK (candidates); i++)
    {
      ...
      if ((BTOR_IS_SYNTH_NODE (real_c)
           || BTOR_IS_BV_CONST_NODE (real_c)
           || btor_find_in_ptr_hash_table (bv_model, real_c))
          && (BTOR_IS_SYNTH_NODE (real_r)
              || BTOR_IS_BV_CONST_NODE (real_r)
              || btor_find_in_ptr_hash_table (bv_model, real_r)))
        {
          // TODO: multiple args
          t = btor_new_bv_tuple (btor->mm, 1);
          index = btor_recursively_compute_assignment (
              btor, bv_model, fun_model, c);
          btor_add_to_bv_tuple (btor->mm, t, index, 0);
          value = btor_recursively_compute_assignment (
              btor, bv_model, fun_model, r);
  ...
  while (has_next_node_hash_table_iterator (&it))
    {
       ...
       * pushed onto 'stack' */
      if (BTOR_IS_UF_NODE (BTOR_REAL_ADDR_NODE (cur)))
        continue;
  ...
  for (i = 0; i < BTOR_COUNT_STACK (stack); i++)
    {
      ...
      if (BTOR_IS_LAMBDA_NODE (cur))
        ...
        {
          bv = btor_recursively_compute_assignment (
      ...
      if ((b = btor_find_in_ptr_hash_table (*bv_model, exp)))
      ...
      else
        {
          ...
          b = btor_insert_in_ptr_hash_table (
                  *bv_model, btor_copy_exp (btor, exp));
          b->data.asPtr = result;
        }
    }
  return result;
...
BtorNode *
btor_generate_lambda_model_from_fun_model (Btor * btor, BtorNode * exp,
                                           const BtorPtrHashTable * model)
{
  ...
  e_else = btor_apply_exps (btor, BTOR_COUNT_STACK (params), params.start,
                            (BtorNode *) uf);
      ...
      for (i = 0; i < args->arity; i++)
        {
          ...
          assert (BTOR_REAL_ADDR_NODE (c)->len ==
                  BTOR_PEEK_STACK (params, i)->len);
          assert (args->arity <= 1
