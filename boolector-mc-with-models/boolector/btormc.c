...
btor_delete_mc_latch (BtorMC * mc, BtorMcLatch * latch)
{
  ...
  if (latch->init)
    boolector_release (btor, latch->init);
  if (latch->next)
    boolector_release (btor, latch->next);
  ...
  while (!BTOR_EMPTY_STACK (*stack))
    {
      ...
      if (node)
        boolector_release (mc->forward, node);
    }
...
btor_mc_release_assignment (BtorMC * mc) 
{
  ...
  if (mc->forward2const)
    {
      BTOR_MSG (boolector_get_btor_msg (mc->btor), 1,
    ...
    if (f->model2const) 
      {
        BTOR_MSG (boolector_get_btor_msg (mc->btor), 1,
          ...
          boolector_count_map (f->model2const), (int)(f - mc->frames.start));
...
boolector_input (BtorMC * mc, int width, const char * name)
{
  ...
  if (name)
    BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
        "declared input %d '%s' of width %d", input->id, name, width);
  else
    BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
        "declared input %d of width %d", input->id, width);
  return res;
}
...
boolector_latch (BtorMC * mc, int width, const char * name)
{
  ...
  if (name)
    BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
        "declared latch %d '%s' of width %d", latch->id, name, width);
  else
    BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
        "declared latch %d of width %d", latch->id, width);
  return res;
}
...
boolector_next (BtorMC * mc, BoolectorNode * node, BoolectorNode * next)
{
  ...
  assert (boolector_get_width (btor, node) ==
          boolector_get_width (btor, next));
...
boolector_constraint (BtorMC * mc, BoolectorNode * constraint)
{
  ...
  BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
            "adding environment CONSTRAINT %d", res);
  return res;
}
...
initialize_inputs_of_frame (BtorMC * mc, BtorMcFrame * f)
{
  ...
  BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
      "initializing %d inputs of frame %d", (int) mc->inputs->count, f->time);
  ...
  while (has_next_hash_table_iterator (&it))
    {
      ...
      if (src)
        {
          dst = 
            boolector_non_recursive_substitute_node (mc->forward, map, src);
          dst = boolector_copy (mc->forward, dst);
...
initialize_constraints_of_frame (BtorMC * mc,
  ...
  BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
      ...
      (int) BTOR_COUNT_STACK (mc->constraints), f->time);
...
initialize_bad_state_properties_of_frame (BtorMC * mc,
  ...
  BTOR_MSG (boolector_get_btor_msg (mc->btor), 2,
      ...
      (int) BTOR_COUNT_STACK (mc->bad), f->time);
  ...
  for (i = 0; i < BTOR_COUNT_STACK (mc->bad); i++)
    {
      if (mc->continue_checking_if_reached || 
          BTOR_PEEK_STACK (mc->reached, i) < 0) 
        {
          ...
          assert (src);
          dst = boolector_non_recursive_substitute_node (mc->forward,
                                                         map, src);
          dst = boolector_copy (mc->forward, dst);
        }
      else
...
map_inputs_and_latches_of_frame (BtorMC * mc, BtorMcFrame * f)
{
  ...
  BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
      "mapping inputs and latchs of frame %d", f->time);
...
initialize_new_forward_frame (BtorMC * mc)
{
  ...
  if (!mc->forward)
    {
      ...
      if (mc->trace_enabled)
        boolector_set_opt (mc->forward, "model_gen", 1);
      if (mc->verbosity)
  ...
  assert (old_mc_btor_num_nodes ==
          mc->btor->nodes_unique_table.num_elements);

  BTOR_MSG (boolector_get_btor_msg (mc->btor), 1, 
      "initialized forward frame at bound k = %d", time);
}
...
check_last_forward_frame (BtorMC * mc)
{
  ...
  BTOR_MSG (boolector_get_btor_msg (mc->btor), 1, 
      "checking forward frame at bound k = %d", k);
  satisfied = 0;
      ...
      if (!bad)
        {
          ...
          assert (reached >= 0);
          BTOR_MSG (boolector_get_btor_msg (mc->btor), 1,
  "skipping checking bad state property %d at bound %d reached before at %d",
            i, k, reached);
          continue;
        }
      BTOR_MSG (boolector_get_btor_msg (mc->btor), 1,
          "checking forward frame bad state property %d at bound k = %d", i, k);
              ...
              if (mc->call_backs.reached_at_bound.fun)
                {
                  mc->call_backs.reached_at_bound.fun (
                    mc->call_backs.reached_at_bound.state,
                    i, k);
                }
            }
        }
...
boolector_bmc (BtorMC * mc, int mink, int maxk)
{
  ...
  BTOR_MSG (boolector_get_btor_msg (mc->btor), 1,
      ...
      (int) BTOR_COUNT_STACK (mc->bad), mink, maxk);

  BTOR_MSG (boolector_get_btor_msg (mc->btor), 1, "trace generation %s",
  ...
  while ((k = BTOR_COUNT_STACK (mc->frames)) <= maxk)
    {
      ...
      if (check_last_forward_frame (mc)) {
        if (mc->stop || 
            mc->num_reached == BTOR_COUNT_STACK (mc->bad) ||
            k == maxk)
        {
          BTOR_MSG (boolector_get_btor_msg (mc->btor), 2, 
              "entering SAT state at bound k=%d", k);
          assert (k >= 0);
          return k;
        }
      }
...
{
  if (!mc->forward2const) 
    mc->forward2const = boolector_new_node_map (mc->btor);
...
btor_mc_forward2const_mapper (Btor * btor, void * state, BoolectorNode * node)
{
  ...
  if (!boolector_is_var (mc->forward, node))
    return 0;
...
btor_mc_forward2const (BtorMC * mc, BoolectorNode * node)
{
  ...
  return boolector_non_recursive_extended_substitute_node (
           mc->btor, map, mc, btor_mc_forward2const_mapper,
           boolector_release, node);
}
...
btor_mc_model2const_mapper (Btor * btor, void * state, BoolectorNode * node)
{
  ...
  if (bucket) 
    ...
    {
      bucket = btor_find_in_ptr_hash_table (mc->latches, node);
      if (!boolector_is_var (mc->btor, node)) sym = 0;
      else sym = boolector_get_symbol (mc->btor, node);
      if (sym)
        BTOR_ABORT_BOOLECTOR (!bucket,
          "variable '%s' not a latch nor an input",
          sym);
      else
        BTOR_ABORT_BOOLECTOR (!bucket,
...
btor_mc_model2const (BtorMC * mc, BoolectorNode * node, int time)
{
  ...
  return boolector_non_recursive_extended_substitute_node (
           mc->btor, map, &mapper, btor_mc_model2const_mapper,
           boolector_release, node);
}
  ...
  if (bucket) 
    {
      ...
      bits_owned_by_forward =
        boolector_bv_assignment (mc->forward, node_at_time);
  ...
  for (i = 0; i < BTOR_COUNT_STACK (mc->constraints); i++)
    {
      ...
      btor_add_constraint_to_dump_context (bdc,
        BTOR_IMPORT_BOOLECTOR_NODE (constraint));
    }
...
boolector_reached_bad_at_bound_mc (BtorMC * mc, int badidx)
{
  ...
  BTOR_ABORT_BOOLECTOR (mc->state == BTOR_NO_MC_STATE,
    "model checker was not run before");
  BTOR_ABORT_BOOLECTOR (mc->stop,
    "stopping at first reached property has to be disabled");
  BTOR_ABORT_BOOLECTOR (badidx < 0,
    "negative bad state property index");
