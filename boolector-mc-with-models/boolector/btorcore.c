...
static void
insert_substitution (Btor * btor, BtorNode * exp, BtorNode * subst,
                          int update)
{
  ...
  if (exp == BTOR_REAL_ADDR_NODE (subst))
    return;
...
report_constraint_stats (Btor * btor, int force)
{
  ...
  if (!force)
    {
      if (btor->options.verbosity.val <= 0)
        return;

      changes = constraints_stats_changes (btor);

      if (btor->options.verbosity.val == 1 && changes < 100000)
        return;

      if (btor->options.verbosity.val == 2 && changes < 1000)
        return;

      if (btor->options.verbosity.val == 3 && changes < 10)
        return;
...
btor_print_stats_btor (Btor * btor)
{
  ...
  if (btor->options.ucopt.val)
    {
      BTOR_MSG (btor->msg, 1, "unconstrained bv props: %d",
                         btor->stats.bv_uc_props);
      BTOR_MSG (btor->msg, 1, "unconstrained array props: %d",
                         btor->stats.fun_uc_props);
    }
#endif
  BTOR_MSG (btor->msg, 1, "variable substitutions: %d",
                     btor->stats.var_substitutions);
  BTOR_MSG (btor->msg, 1, "uninterpreted function substitutions: %d",
                     btor->stats.uf_substitutions);
  BTOR_MSG (btor->msg, 1, "embedded constraint substitutions: %d",
                     btor->stats.ec_substitutions);
  BTOR_MSG (btor->msg, 1, "assumptions: %u", btor->assumptions->count);

  if (btor->ops[BTOR_FEQ_NODE].cur)
    BTOR_MSG (btor->msg, 1,
      "virtual reads: %d", btor->stats.vreads);

  if (verbosity > 0)
    {
      BTOR_MSG (btor->msg, 2, "max rec. RW: %d", btor->stats.max_rec_rw_calls);
      BTOR_MSG (btor->msg, 2, "number of expressions ever created: %lld",
      ...
      BTOR_MSG (btor->msg, 1, "memory allocated for nodes: %.2f MB",
              btor->stats.node_bytes_alloc / (double) (1 << 20));
      if (num_final_ops > 0)
        for (i = 1; i < BTOR_NUM_OPS_NODE - 1; i++)
          if (btor->ops[i].cur || btor->ops[i].max)
            BTOR_MSG (btor->msg, 2, " %s: %d max %d",
              g_btor_op2string[i], btor->ops[i].cur, btor->ops[i].max);
    }
  ...
  if (btor->stats.lod_refinements)
    {
      BTOR_MSG (btor->msg, 1, " function congruence conflicts: %d",
                         btor->stats.function_congruence_conflicts);
      BTOR_MSG (btor->msg, 1, " beta reduction conflicts: %d",
                         btor->stats.beta_reduction_conflicts);
      BTOR_MSG (btor->msg, 1, " average lemma size: %.1f",
      ...
      for (i = 1; i < BTOR_SIZE_STACK (btor->stats.lemmas_size); i++)
        {
          ...
          BTOR_MSG (btor->msg, 1, "   lemmas of size %d: %d", i, 
              btor->stats.lemmas_size.start[i]);
        }
      BTOR_MSG (btor->msg, 1, " average linking clause size: %.1f",
                         BTOR_AVERAGE_UTIL (btor->stats.lclause_size_sum,
                                            btor->stats.lod_refinements));
    }
  BTOR_MSG (btor->msg, 1, "");

  BTOR_MSG (btor->msg, 1, "linear constraint equations: %d",
    btor->stats.linear_equations);
  BTOR_MSG (btor->msg, 1, "gaussian elimination in linear equations: %d",
    btor->stats.gaussian_eliminations);
  BTOR_MSG (btor->msg, 1, "eliminated sliced variables: %d",
    btor->stats.eliminated_slices);
  BTOR_MSG (btor->msg, 1, "extracted skeleton constraints: %d",
    btor->stats.skeleton_constraints);
  BTOR_MSG (btor->msg, 1, "and normalizations: %d", btor->stats.ands_normalized);
  BTOR_MSG (btor->msg, 1, "add normalizations: %d", btor->stats.adds_normalized);
  BTOR_MSG (btor->msg, 1, "mul normalizations: %d", btor->stats.muls_normalized);
  BTOR_MSG (btor->msg, 1, "synthesis assignment inconsistencies: %d",
                btor->stats.synthesis_assignment_inconsistencies);
  BTOR_MSG (btor->msg, 1, "  apply nodes: %d",
                btor->stats.synthesis_inconsistency_apply);
  BTOR_MSG (btor->msg, 1, "  lambda nodes: %d",
                btor->stats.synthesis_inconsistency_lambda);
  BTOR_MSG (btor->msg, 1, "  var nodes: %d",
                btor->stats.synthesis_inconsistency_var);

  BTOR_MSG (btor->msg, 1, "apply propagation during construction: %d",
                btor->stats.apply_props_construct);
  BTOR_MSG (btor->msg, 1, "beta reductions: %lld",
                btor->stats.beta_reduce_calls);
  BTOR_MSG (btor->msg, 1, "expression evaluations: %lld",
                btor->stats.eval_exp_calls);
  BTOR_MSG (btor->msg, 1, "synthesized lambda applies: %lld", 
  ...
  BTOR_MSG (btor->msg, 1, "propagations down: %lld",
                btor->stats.propagations_down);
  BTOR_MSG (btor->msg, 1, "partial beta reduction restarts: %lld",
            btor->stats.partial_beta_reduction_restarts);

  if (btor->options.dual_prop.val)
    {
      BTOR_MSG (btor->msg, 1, "dual prop. vars (failed/assumed): %d/%d",
                    btor->stats.dp_failed_vars,
                    btor->stats.dp_assumed_vars);
      BTOR_MSG (btor->msg, 1, "dual prop. applies (failed/assumed): %d/%d",
  ...
  BTOR_MSG (btor->msg, 1, "%.2f seconds expression evaluation", btor->time.eval);
  BTOR_MSG (btor->msg, 1, "%.2f seconds synthesize expressions",
            btor->time.synth_exp);
  BTOR_MSG (btor->msg, 1, "%.2f seconds lazy apply encoding",
                btor->time.enc_app);
  BTOR_MSG (btor->msg, 1, "%.2f seconds lazy lambda encoding",
                btor->time.enc_lambda);
  BTOR_MSG (btor->msg, 1, "%.2f seconds lazy var encoding",
                btor->time.enc_var);
  BTOR_MSG (btor->msg, 1, "%.2f seconds node search", btor->time.find_dfs);
  BTOR_MSG (btor->msg, 1, "%.2f seconds reachable search", btor->time.reachable);
  BTOR_MSG (btor->msg, 1, "%.2f seconds initial applies search", 
  ...
  if (btor->options.dual_prop.val)
    {
      BTOR_MSG (btor->msg, 1, "%.2f seconds cloning for initial applies search",                 btor->time.search_init_apps_cloning);
      BTOR_MSG (btor->msg, 1, 
  ...
  BTOR_MSG (btor->msg, 1, "%.2f seconds determining failed assumptions",
            btor->time.failed);
  BTOR_MSG (btor->msg, 1, "%.2f seconds lemma generation", btor->time.lemma_gen);
  BTOR_MSG (btor->msg, 1, "%.2f seconds not encoded apply search",
            btor->time.find_nenc_app);
  BTOR_MSG (btor->msg, 1, "%.2f seconds propagation apply search",
            btor->time.find_prop_app);
  if (btor->options.dual_prop.val)
    BTOR_MSG (btor->msg, 1, "%.2f seconds propagation apply in conds search",
              btor->time.find_cond_prop_app);
  BTOR_MSG (btor->msg, 1, "%.2f seconds for cloning",
                btor->time.cloning);
  BTOR_MSG (btor->msg, 1, "%.2f seconds beta reduction probing",
            btor->time.br_probing);
#ifndef BTOR_DO_NOT_OPTIMIZE_UNCONSTRAINED
  if (btor->options.ucopt.val)
    BTOR_MSG (btor->msg, 1, "%.2f seconds for unconstrained optimization",
        btor->time.ucopt);
#endif
  if (btor->options.model_gen.val)
    BTOR_MSG (btor->msg, 1, "%.2f seconds model generation", btor->time.model_gen);
  BTOR_MSG (btor->msg, 1, "");
  BTOR_MSG (btor->msg, 1, "%.2f seconds in rewriting engine",
                btor->time.rewrite);
  ...
  BTOR_MSG (btor->msg, 1,
    ...
    btor->time.embedded, percent (btor->time.embedded, btor->time.rewrite));
  BTOR_MSG (btor->msg, 1,
    "%.2f seconds in beta reduction during rewriting (%.0f%%)",
    btor->time.betareduce, percent (btor->time.betareduce, btor->time.rewrite));
  if (btor->options.eliminate_slices.val)
    BTOR_MSG (btor->msg, 1, "%.2f seconds in slicing during rewriting (%.0f%%)",
...
#endif
  BTOR_MSG (btor->msg, 1, "%.1f MB", btor->mm->maxallocated / (double) (1 << 20));
}
...
new_aux_btor (int init_opts)
{
  ...
  btor->symbols = btor_new_ptr_hash_table (mm, 
      (BtorHashPtr) btor_hash_str, (BtorCmpPtr) strcmp);
  btor->node2symbol = btor_new_ptr_hash_table (mm,
...
release_all_ext_exp_refs (Btor * btor)
{
  ...
  for (i = BTOR_COUNT_STACK (btor->nodes_id_table) - 1; i >= 0; i--)
    {
      if (!(exp = BTOR_PEEK_STACK (btor->nodes_id_table, i)))
        continue;
      if (exp->ext_refs)
        {
...
release_all_ext_sort_refs (Btor * btor)
{
  ...
  for (i = BTOR_COUNT_STACK (btor->sorts_unique_table.id2sort) - 1;
       i >= 0; i--)
    {
...
btor_delete_btor (Btor * btor)
{
  ...
  if (btor->parse_error_msg)
    btor_freestr (mm, btor->parse_error_msg);
      ...
      while (has_next_node_hash_table_iterator (&it))
        {
          if (btor->options.just_heuristic.val
              == BTOR_JUST_HEUR_BRANCH_MIN_APP)
            {
              ...
              while (has_next_node_hash_table_iterator (&iit))
                btor_release_exp (
                    btor, next_node_hash_table_iterator (&iit));
              btor_delete_ptr_hash_table (t);
            }
          else
            {
...
update_assumptions (Btor * btor)
{
  ...
  ass = btor_new_ptr_hash_table (btor->mm, (BtorHashPtr) btor_hash_exp_by_id, 
                                 (BtorCmpPtr) btor_compare_exp_by_id);
  ...
  if (!bucket)
    {
      if (!BTOR_IS_FUN_NODE (BTOR_REAL_ADDR_NODE (right)) &&
        ...
        {
          btor_insert_in_ptr_hash_table (btor->var_rhs,
      ...
      if (BTOR_IS_FUN_NODE (BTOR_REAL_ADDR_NODE (right)) &&
        ...
        {
          btor_insert_in_ptr_hash_table (btor->fun_rhs,
                                         btor_copy_exp (btor, left));
        }

      BTORLOG ("varsubst: %s -> %s", node2string (left), node2string (right));
      btor_insert_in_ptr_hash_table (vsc,
        btor_copy_exp (btor, left))->data.asPtr = btor_copy_exp (btor, right);
                        ...
                        BtorNode ** left_result, BtorNode ** right_result)
{
  ...
  if (!BTOR_IS_BV_VAR_NODE (real_left) &&
      ...
      !BTOR_IS_UF_NODE (real_left) &&
      !BTOR_IS_UF_NODE (real_right))
    {
...
constraint_is_inconsistent (Btor * btor, BtorNode * exp)
{
  ...
  return rep == BTOR_INVERT_NODE (rep)
         ...
         || btor_find_in_ptr_hash_table (
                btor->synthesized_constraints, BTOR_INVERT_NODE (rep))
         || btor_find_in_ptr_hash_table (
                btor->unsynthesized_constraints, BTOR_INVERT_NODE (rep))
         || btor_find_in_ptr_hash_table (
                btor->embedded_constraints, BTOR_INVERT_NODE (rep));
}
...
btor_assume_exp (Btor * btor, BtorNode * exp)
{
  ...
  if (!btor_find_in_ptr_hash_table (btor->assumptions, exp))
    (void) btor_insert_in_ptr_hash_table (
        btor->assumptions, btor_copy_exp (btor, exp));
}
          ...
          for (i = 0; i < 2; i++)
            {
              ...
              if (!BTOR_IS_INVERTED_NODE (e) && BTOR_IS_AND_NODE (e))
                ...
                {
                  if (!BTOR_IS_SYNTH_NODE (BTOR_REAL_ADDR_NODE (e)))
                    continue;
...
set_simplified_exp (Btor * btor, BtorNode * exp, BtorNode * simplified)
{
  ...
  if (BTOR_REAL_ADDR_NODE (simplified)->parameterized)
    exp->parameterized = 1;
}
  ...
  if (btor->embedded_constraints->count > 0u)
    {
          ...
          if (btor_find_in_ptr_hash_table (btor->embedded_constraints, cur))
            {
              ...
              btor_remove_from_ptr_hash_table (
                  btor->embedded_constraints, cur, 0, 0);
      ...
      BTOR_MSG (btor->msg, 1, "replaced %d embedded constraints in %1.f seconds",
                count, delta);
    }
}
...
merge_lambdas (Btor * btor)
{
  ...
  while (has_next_node_hash_table_iterator (&it))
    {
      ...
      if (!parent->parameterized && BTOR_IS_APPLY_NODE (parent))
        continue;
  ...
  for (i = 0; i < BTOR_COUNT_STACK (stack); i++)
    {
      ...
      if (BTOR_IS_LAMBDA_NODE (lambda->first_parent))
        continue;
      ...
      while (!BTOR_EMPTY_STACK (visit))
        {
          ...
          while (has_next_parent_full_parent_iterator (&nit))
            BTOR_PUSH_STACK (mm, visit,
                             next_parent_full_parent_iterator (&nit));
        }
  ...
  assert (check_unique_table_merge_unset_dbg (btor));
  delta = btor_time_stamp () - start;
  BTOR_MSG (btor->msg, 1, "merged %d lambdas in %.2f seconds", delta_lambdas,
            delta);
}
...
optimize_unconstrained (Btor * btor)
{
  ...
  ucs = btor_new_ptr_hash_table (mm, (BtorHashPtr) btor_hash_exp_by_id,
                                 (BtorCmpPtr) btor_compare_exp_by_id);
  init_substitutions (btor);
          ...
          if (!cur->parents)
          ...
          else
            {
              ...
              while (has_next_parent_full_parent_iterator (&pit))
                BTOR_PUSH_STACK (
                    mm, stack, next_parent_full_parent_iterator (&pit));
            }
        }
    }
          ...
          while (has_next_parameterized_iterator (&parit))
            {
              ...
              lambda = BTOR_PARAM_GET_LAMBDA_NODE (
          ...
          if (cur->parents == 0 || (cur->parents == 1 && !cur->constraint))
            {
              for (i = cur->arity - 1; i >= 0; i--)
                uc[i] = (btor_find_in_ptr_hash_table (
                          ucs, BTOR_REAL_ADDR_NODE (cur->e[i]))) ? 1 : 0;
...
//  if (btor->options.rewrite_level.val <= 1 && !btor->options.beta_reduce_all.val)
//    return;
      ...
      if (cur->synth_mark == 0)
        {
          if (BTOR_IS_BV_CONST_NODE (cur))
            ...
            {
PUSH_CHILDREN:
              assert (!btor->options.lazy_synthesize.val
  ...
  if (count > 0 && btor->options.verbosity.val > 3)
    BTOR_MSG (btor->msg, 3, "synthesized %u expressions into AIG vectors", count);
  ...
  while (has_next_node_map_iterator (&it))
    {
      ...
      if (btor_failed_exp (clone, bv_eq))
        {
          BTORLOG ("failed: %s (%s)", node2string (cur_btor),
                         ...
                         int (* dp_cmp_inputs) (const void *, const void *))
{
  ...
  qsort (inputs->start, BTOR_COUNT_STACK (*inputs), sizeof (BtorNode *),
         dp_cmp_inputs);
                                  ...
                                  BtorNodePtrStack * top_applies)
{
  ...
  set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                           top_applies, btor_compare_scores_qsort);
#elif DP_QSORT == DP_QSORT_ASC
  set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                           top_applies, btor_cmp_exp_by_id_qsort_asc);
#elif DP_QSORT == DP_QSORT_DESC
  set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                           top_applies, btor_cmp_exp_by_id_qsort_desc);
#else
  ...
  if (!btor->dp_cmp_inputs)
#endif
    {
      ...
      set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                               &tmp_desc, btor_cmp_exp_by_id_qsort_desc);
      set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                               &tmp_asc, btor_cmp_exp_by_id_qsort_asc);
      ...
      if (BTOR_COUNT_STACK (tmp_asc) < BTOR_COUNT_STACK (tmp_desc))
        {
          ...
          for (i = 0; i < BTOR_COUNT_STACK (tmp_asc); i++)
            BTOR_PUSH_STACK (
                btor->mm, *top_applies, BTOR_PEEK_STACK (tmp_asc, i));
        }
      else
        {
          ...
          for (i = 0; i < BTOR_COUNT_STACK (tmp_desc); i++)
            BTOR_PUSH_STACK (
                btor->mm, *top_applies, BTOR_PEEK_STACK (tmp_desc, i));
        }
    ...
    set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                             top_applies, btor->dp_cmp_inputs);
#endif
#endif
      ...
      while (has_next_hash_table_iterator (&it))
        {
          ...
          if (BTOR_REAL_ADDR_NODE (cond)->parameterized)
            {
              i = 0;
              init_parameterized_iterator (btor, &pit,
              ...
              while (has_next_parameterized_iterator (&pit))
                {
                  ...
                  btor_assign_param (btor,
                      BTOR_PARAM_GET_LAMBDA_NODE (param), arg);
                }
  ...
  if (app1)
    {
      ...
      add_symbolic_lemma (btor, bconds_sel1, bconds_sel2,
...
static void
find_not_encoded_applies_vars (Btor * btor, BtorNode * exp,
  ...
  do
    {
      ...
      if (cur->mark || cur->tseitin || BTOR_IS_FUN_NODE (cur))
        continue;
...
static int
encode_applies_vars (Btor * btor, BtorLambdaNode * lambda,
                     BtorNodePtrStack * param_apps)
{
  assert (btor);
  assert (lambda);
...
static void
push_applies_for_propagation (Btor * btor, BtorNode * exp,
...
propagate (Btor * btor, BtorNodePtrStack * prop_stack,
           ...
           BtorIntHashTable * apply_search_cache, int * assignments_changed)
{
  assert (btor);
  assert (prop_stack);
  ...
  to_prop = btor_new_ptr_hash_table (mm, (BtorHashPtr) btor_hash_exp_by_id,
                                         (BtorCmpPtr) btor_compare_exp_by_id);
  conds = check_conds
          ? btor_new_ptr_hash_table (mm, (BtorHashPtr) btor_hash_exp_by_id,
                                          (BtorCmpPtr) btor_compare_exp_by_id)
                  ...
                  if (check_conds)
                    {
                      ...
                      while (has_next_node_hash_table_iterator (&it))
                        btor_release_exp (
                            btor, next_node_hash_table_iterator (&it));
                      btor_delete_ptr_hash_table (conds);
                    }
                  return 1;
                }
          ...
          while (has_next_node_hash_table_iterator (&it))
            {
              ...
              if (param_app != BTOR_REAL_ADDR_NODE (fun_value)
                  ...
                  || BTOR_IS_INVERTED_NODE (fun_value)
                  || fun_value->e[1] != args)
                {
      ...
      if (!BTOR_REAL_ADDR_NODE (fun_value)->tseitin)
        {
          ...
          if (!BTOR_IS_INVERTED_NODE (fun_value)
              && BTOR_IS_APPLY_NODE (fun_value))
          ...
          if (!args_equal)
            {
              ...
              *assignments_changed =
                encode_applies_vars (btor, lambda, &param_apps);
...
BETA_REDUCTION_CONFLICT:
              ...
              push_applies_for_propagation (btor, fun_value, lambda,
                                            prop_stack, apply_search_cache);
              if (check_conds)
                {
                  init_node_hash_table_iterator (&it, conds);
                  while (has_next_node_hash_table_iterator (&it))
...
BTOR_CONFLICT_CHECK:
  ...
  qsort (top_applies.start, BTOR_COUNT_STACK (top_applies), sizeof (BtorNode *),
      ...
      else
        {
          ...
          if (found_conflict || changed_assignments)
          ...
          else
            {
              ...
              BTOR_PUSH_STACK (mm, btor->functions_with_model, 
...
sat_aux_btor (Btor * btor, int lod_limit, int sat_limit)
{
   ...
   * eliminated */
  if (!btor->options.incremental.val
      && smgr->inc_required
...
UNSAT:
  ...
  while (has_next_node_hash_table_iterator (&it))
    assert (!BTOR_REAL_ADDR_NODE (
                 next_node_hash_table_iterator (&it))->simplified);
#endif
  ...
  while (sat_result == BTOR_SAT)
    {
      ...
      if (clone) 
        add_lemma_to_dual_prop_clone (btor, clone, &clone_root, exp_map);
...
br_probe (Btor * btor)
{
  ...
  BTOR_MSG (btor->msg, 1, "  number of nodes: %d/%d (factor: %.1f, max: %d)",
            num_ops_orig, num_ops_clone, (float) num_ops_clone / num_ops_orig,
  ...
  if (res != BTOR_UNKNOWN)
  ...
  else if (num_ops_clone < num_ops_orig * btor->options.pbra_ops_factor.val)
    {
      ...
      res = sat_aux_btor (clone, btor->options.pbra_lod_limit.val, 
          btor->options.pbra_sat_limit.val);
      btor_delete_btor (clone);
    }
  ...
  if (res != BTOR_UNKNOWN)
    {
      delta = btor_time_stamp () - start;
      BTOR_MSG (btor->msg, 1,
                "  probing succeeded (%.2f seconds)", delta);
      btor->time.br_probing += delta;
      return res;
    }
...
#ifdef BTOR_ENABLE_BETA_REDUCTION_PROBING
  if (btor_has_clone_support_sat_mgr (btor_get_sat_mgr_btor (btor))
      && btor->options.probe_beta_reduce_all.val
      && lod_limit == -1 
      && sat_limit == -1)
    {
      res = br_probe (btor);
...
{
  exp = BTOR_REAL_ADDR_NODE (exp);
  if (btor_is_fun_exp (btor, exp))
    return 0;
  if (btor_is_array_exp (btor, exp))
    return 0;
  /* scope of bound parmaters already closed (cannot be used anymore) */
  if (BTOR_IS_PARAM_NODE (exp) && BTOR_IS_BOUND_PARAM_NODE (exp))
    return 0;
  return 1;
}
          ...
          switch (real_cur->kind)
            {
              case BTOR_SLICE_NODE:
                result =
                  btor_slice_bv (
                      btor->mm, e[0], real_cur->upper, real_cur->lower);
  ...
  while (has_next_node_hash_table_iterator (&it))
    {
      ...
      btor_insert_in_ptr_hash_table (inputs,
          btor_copy_exp (clone, cur))->data.asPtr =
...
rebuild_formula (Btor * btor, int rewrite_level)
{
  ...
  t = btor_new_ptr_hash_table (btor->mm, (BtorHashPtr) btor_hash_exp_by_id,
                                         (BtorCmpPtr) btor_compare_exp_by_id);

  /* collect all leaves and rebuild whole formula */
  for (i = BTOR_COUNT_STACK (btor->nodes_id_table) - 1; i >= 0; i--)
    {
      if (!(cur = BTOR_PEEK_STACK (btor->nodes_id_table, i)))
        continue;
...
check_model (Btor * btor, Btor * clone, BtorPtrHashTable * inputs)
{
  ...
  if (clone->valid_assignments)
    reset_incremental_usage (clone);
...
check_failed_assumptions (Btor * btor, Btor * clone)
{
  ...
  while (has_next_node_hash_table_iterator (&it))
    btor_release_exp (clone, next_node_hash_table_iterator (&it));
  btor_delete_ptr_hash_table (clone->assumptions);
  clone->assumptions = btor_new_ptr_hash_table (
