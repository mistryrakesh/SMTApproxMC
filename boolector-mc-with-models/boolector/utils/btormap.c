...
static BtorNode * btor_never_map_mapper (
  Btor * btor,
  void * state,
  BtorNode * node)
{
...
BtorNode *
btor_non_recursive_substitute_node (
  Btor * btor,
  BtorNodeMap * map,
  BtorNode * root)
{
...
BoolectorNode *
boolector_non_recursive_extended_substitute_node (
  Btor * btor,
...
BoolectorNode *
boolector_non_recursive_substitute_node (
  Btor * btor,
  BoolectorNodeMap * map,
  BoolectorNode * root)
{
...
void btor_map_aig (BtorAIGMap * map, BtorAIG * src, BtorAIG * dst)
{
  assert (map);
  assert (src);
  assert (dst);
