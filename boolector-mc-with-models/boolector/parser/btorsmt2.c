...
void boolector_print_value (Btor * btor, BoolectorNode * node, char * node_str,
...
typedef struct BtorSMT2Item
{
  ...
  union { BtorSMT2Node * node; BoolectorNode * exp; char * str; };
} BtorSMT2Item;
  ...
  if (!parser->error)
    {
      ...
      va_start (ap, fmt);
      parser->error =
        btor_parse_error_message (parser->mem, parser->infile_name,
...
static BtorSMT2Node *
btor_new_node_smt2 (BtorSMT2Parser * parser,
                                          BtorSMT2Tag tag)
{
  BtorSMT2Node * res;
...
static void
btor_release_symbol_smt2 (BtorSMT2Parser * parser,
                                      BtorSMT2Node * symbol)
{
...
static void
btor_remove_symbol_smt2 (BtorSMT2Parser * parser,
                                     BtorSMT2Node * symbol)
{
  BtorSMT2Node ** p;
  p = btor_symbol_position_smt2 (parser, symbol->name);
        ...
        if (ch == EOF)
          {
            ...
            return !btor_perr_smt2 (
                        parser, "unexpected end-of-file in comment");
          }
    goto RESTART;
  }
      ...
      if (ch == 'b')
        {
          ...
          if((ch = btor_nextch_smt2 (parser)) == EOF)
            return !btor_perr_smt2 (
                        parser, "unexpected end-of-file after '#b'");
          if (ch != '0' && ch != '1')
      ...
      else if (ch == 'x')
        {
          ...
          if ((ch = btor_nextch_smt2 (parser)) == EOF)
            return !btor_perr_smt2 (
                        parser, "unexpected end-of-file after '#x'");
      ...
      for (;;)
        {
          if ((ch = btor_nextch_smt2 (parser)) == EOF)
            return !btor_cerr_smt2 (
                        parser, "unexpected", ch, "in quoted symbol");
          if (ch == '|')
            {
              btor_pushch_smt2 (parser, '|');
              btor_pushch_smt2 (parser, 0);
      ...
      if (ch == '.')
        {
          ...
          if ((ch = btor_nextch_smt2 (parser)) == EOF)
            return !btor_perr_smt2 (
                        parser, "unexpected end-of-file after '0.'");
          if (!(btor_cc_smt2 (parser, ch) & BTOR_DECIMAL_DIGIT_CHAR_CLASS_SMT2))
            return !btor_perr_smt2 (
                        parser, "expected decimal digit after '0.'");
          btor_pushch_smt2 (parser, ch);
          for (;;)
            {
              ch = btor_nextch_smt2 (parser);
              if (!(btor_cc_smt2 (parser, ch) 
                    & BTOR_DECIMAL_DIGIT_CHAR_CLASS_SMT2))
                break;
              btor_pushch_smt2 (parser, ch);
            }
        }
  ...
  for (i = 1; i <= nargs; i++)
    {
      ...
      if (boolector_is_fun (parser->btor, p[i].exp))
        {
          parser->perrcoo = p[i].coo;
          return !btor_perr_smt2 (parser, "argument %d of '%s' is a function",
                                  i, p->node->name);
        }
    }
  return 1;
...
static BoolectorNode *
btor_translate_rotate_smt2 (Btor * btor, 
                            BoolectorNode * exp, 
                            int shift, 
                            int left)
{
      ...
      if (tag == BTOR_RPAR_TAG_SMT2)
        {
          ...
          if (tag == BTOR_EXP_TAG_SMT2
              && nargs && boolector_is_fun (parser->btor, p[0].exp))
            {
              ...
              i = boolector_fun_sort_check (
                      parser->btor, fargs.start, nargs, tmp);
              if (i >= 0)
                {
                  BTOR_RELEASE_STACK (parser->mem, fargs);
                  return !btor_perr_smt2 (parser,
...
BIN_BOOL_LEFT_ASSOCIATIVE_CORE:
            ...
            {
              binfun = boolector_or; goto BIN_BOOL_LEFT_ASSOCIATIVE_CORE;
            }
          else if (tag == BTOR_XOR_TAG_SMT2)
            {
              binfun = boolector_xor; goto BIN_BOOL_LEFT_ASSOCIATIVE_CORE;
            }
            ...
            if (tag == BTOR_ITE_TAG_SMT2)
            ...
            else if (tag == BTOR_EXTRACT_TAG_SMT2)
              {
                ...
                if (!btor_check_not_array_or_uf_args_smt2 (parser, p, nargs))
                  return 0;
                width = boolector_get_width (parser->btor, p[1].exp);
                if (width <= p->hi)
                  {
...
BIN_BV_LEFT_ASSOCIATIVE_CORE:
              ...
              {
                binfun = boolector_and; goto BIN_BV_LEFT_ASSOCIATIVE_CORE;
              }
            else if (tag == BTOR_BVOR_TAG_SMT2)
              {
                binfun = boolector_or; goto BIN_BV_LEFT_ASSOCIATIVE_CORE;
              }
            else if (tag == BTOR_BVXOR_TAG_SMT2)
              {
                binfun = boolector_xor; goto BIN_BV_LEFT_ASSOCIATIVE_CORE;
              }
            else if (tag == BTOR_BVADD_TAG_SMT2)
              {
                binfun = boolector_add; goto BIN_BV_LEFT_ASSOCIATIVE_CORE;
              }
            else if (tag == BTOR_BVSUB_TAG_SMT2)
              {
                binfun = boolector_sub; goto BIN_BV_LEFT_ASSOCIATIVE_CORE;
              }
            else if (tag == BTOR_BVMUL_TAG_SMT2)
              {
                binfun = boolector_mul; goto BIN_BV_LEFT_ASSOCIATIVE_CORE;
              }
          ...
          if (tag == BTOR_LPAR_TAG_SMT2)
              ...
              if (parser->binding)
                {
                  ...
                  for (i = 0;
                       tokens && i < BTOR_COUNT_STACK (parser->token);
                       i++)
                    {
                      c = BTOR_PEEK_STACK (parser->token, i);
                      BTOR_PUSH_STACK (parser->btor->mm, *tokens, c ? c : ' ');
          ...
          else if (btor_item_with_node_smt2 (p))
            {
              ...
              if (tag & BTOR_COMMAND_TAG_CLASS_SMT2)
                return !btor_perr_smt2 (parser,
                    "unexpected command '%s'", p->node->name);
              if (tag & BTOR_KEYWORD_TAG_CLASS_SMT2)
                return !btor_perr_smt2 (parser,
                    "unexpected keyword '%s'", p->node->name);
              if (tag & BTOR_LOGIC_TAG_CLASS_SMT2)
                return !btor_perr_smt2 (parser,
                    "unexpected logic '%s'", p->node->name);
              if (tag & BTOR_RESERVED_TAG_CLASS_SMT2)
                {
                  if (tag == BTOR_LET_TAG_SMT2)
                    {
                      if (!btor_read_lpar_smt2 (parser, " after 'let'"))
                        return 0;
...
ONE_FIXED_NUM_PARAMETRIC:
                          ...
                          if (BTOR_COUNT_STACK (parser->work) < 3)
                            {
                              assert (BTOR_COUNT_STACK (parser->work) == 2);
                              assert (parser->work.start[0].tag 
                                      == BTOR_LPAR_TAG_SMT2);
                              assert (parser->work.start[1].tag
                                      == BTOR_UNDERSCORE_TAG_SMT2);
                              parser->perrcoo = parser->work.start[0].coo;
                              return !btor_perr_smt2 (parser,
                                  "expected another '(' before '(_ %s'", 
                                  node->name);
                            }
                          ...
                          l = p - 1;
                          if (!btor_parse_int32_smt2 (
                                   parser, 0, &l->num, tokens))
                            return 0;
                          l->tag = tag;
                          l->node = node;
                          parser->work.top = p;
                          if (!btor_read_rpar_smt2 (parser,  read_rpar_msg))
                            return 0;
                          if (tokens)
                            BTOR_PUSH_STACK (parser->btor->mm, *tokens, ')');
                          assert (open > 0);
                      ...
                      else if (tag == BTOR_EXTRACT_TAG_SMT2)
                        {
                          ...
                          l = p - 1;
                          if (!btor_parse_int32_smt2 (
                                   parser, 0, &l->hi, tokens))
                            return 0;
                          firstcoo = parser->coo;
                          if (!btor_parse_int32_smt2 (
                                   parser, 0, &l->lo, tokens))
                            return 0;
                          if (l->hi < l->lo)
                            {
                               ...
                               && btor_bvconst_str_smt2 (parser->token.start))
                        {
                          ...
                          BtorSMT2Coo coo;
                             ...
                             exp = 0;
                             decstr = btor_strdup (
                              parser->mem, parser->token.start + 2);
                             constr = btor_decimal_to_const (
                              parser->mem, parser->token.start + 2);
                             coo = parser->coo;
                             coo.y += 2;
                             if (!btor_parse_int32_smt2 (
                                   parser, 1, &width, tokens))
                            ...
                            {
                              char * uconstr = btor_uext_const (
                                  parser->mem, constr, width - len);
...
UNDERSCORE_DONE:
                          ...
                          if (!exp) return 0;
                          assert (boolector_get_width (parser->btor, exp) 
                                  == width);
                          ...
                          p->exp = exp;
                          if (!btor_read_rpar_smt2 (
                                parser, " to close '(_ bv..'")) return 0;
                          if (tokens)
                            BTOR_PUSH_STACK (parser->btor->mm, *tokens, ')');
                        }
                      else
...
btor_parse_bitvec_sort_smt2 (BtorSMT2Parser * parser, int skiplu, int * resptr)
{
  ...
  *resptr = res;
  BTOR_MSG (boolector_get_btor_msg (parser->btor), 3, 
      "parsed bit-vector sort of width %d", res);
      ...
      if (tag != BTOR_UNDERSCORE_TAG_SMT2)
        {
          ...
          return !btor_perr_smt2 (parser, " exptected '_'");
        }
      if (!btor_parse_bitvec_sort_smt2 (parser, 1, &width)) 
        {
          BTOR_RELEASE_STACK (parser->mem, args);
          return 0;
        }
...
//    return 0;
  ...
  BITVEC:
      if (!BTOR_EMPTY_STACK (args))
        {
              ...
              for (i = 0; i < BTOR_COUNT_STACK (args); i++)
                sdomain[i] =
                  boolector_bitvec_sort (
                      parser->btor, BTOR_PEEK_STACK (args, i));
            }
          else
            sdomain[0] =
          ...
          sort = boolector_fun_sort (parser->btor, sdomain,
                                     BTOR_COUNT_STACK (args), scodomain);
          ...
          BTOR_MSG (boolector_get_btor_msg (parser->btor), 2,
  ...
  else if (tag == BTOR_ARRAY_TAG_SMT2)
    {
      ...
      if (!btor_parse_bitvec_sort_smt2 (parser, 0, &domain))
        {
          BTOR_RELEASE_STACK (parser->mem, args);
          return 0;
        }
      if (!btor_parse_bitvec_sort_smt2 (parser, 0, &width))
        {
          BTOR_RELEASE_STACK (parser->mem, args);
          return 0;
        }
    ...
    {
      BTOR_RELEASE_STACK (parser->mem, args);
      return !btor_perr_smt2 (parser, "expected '_' or 'Array' at '%s'",
                              parser->token.start);
    }
...
SORTED_VAR:
  ...
  if (tag == BTOR_LPAR_TAG_SMT2)  /* parse function arguments */
    {
  ...
  if (tag == EOF)
    return !btor_perr_smt2 (
        parser, "reached end-of-file expecting '_' or 'Array'");
  if (tag == BTOR_UNDERSCORE_TAG_SMT2)
    {
...
BITVEC:
      ...
      else
        {
          if (boolector_get_width (parser->btor, fun->exp) != width)
            return !btor_perr_smt2 (parser,
              ...
              boolector_get_width (parser->btor, fun->exp), width);
          BTOR_MSG (boolector_get_btor_msg (parser->btor), 2,
  ...
  else if (tag == BTOR_ARRAY_TAG_SMT2)
    {
      if (nargs)
        return !btor_perr_smt2 (
            parser, "sort Array is not supported for arity > 0");
      assert (!domain);
      assert (!width);
      if (parser->commands.set_logic && parser->res->logic == BTOR_LOGIC_QF_BV)
        return !btor_perr_smt2 (parser, "'Array' invalid for logic 'QF_BV'");
      ...
      else
        {
          ...
          if (!boolector_get_index_width (parser->btor, fun->exp) != domain)
            return !btor_perr_smt2 (parser,
                ...
                boolector_get_index_width (parser->btor, fun->exp), domain);
          if (!boolector_get_width (parser->btor, fun->exp) != width)
            return !btor_perr_smt2 (parser,
  ...
  if (boolector_get_width (parser->btor, exp) != width)
    {
      ...
      return !btor_perr_smt2 (
          parser, "invalid term bit-width, expected %d but was %d",
          width, domain);
    }
...
INVALID_STATUS_VALUE:
      ...
      BTOR_MSG (boolector_get_btor_msg (parser->btor), 2, 
          "parsed status '%s'", parser->token.start);
...
btor_set_option_smt2 (BtorSMT2Parser * parser)
{
  ...
  if (tag == EOF)
    return !btor_perr_smt2 (
        parser, "unexpected end-of-file after 'set-option'");
  if (tag == BTOR_RPAR_TAG_SMT2)
      ...
      switch (tag)
        {
          ...
          case BTOR_OPT_MODEL_GEN_TAG_SMT2:
            opt = BTOR_OPT_MODEL_GEN;
            break;
          case BTOR_VERBOSITY_TAG_SMT2:
            opt = BTOR_OPT_VERBOSITY;
            verb = 1;
            break;
          case BTOR_OPT_INCREMENTAL_TAG_SMT2:
            opt = BTOR_OPT_INCREMENTAL;
            break;
          case BTOR_OPT_INPUT_FORMAT_TAG_SMT2:
            opt = BTOR_OPT_INPUT_FORMAT;
            break;
          case BTOR_OPT_OUTPUT_NUMBER_FORMAT_TAG_SMT2:
            opt = BTOR_OPT_OUTPUT_NUMBER_FORMAT;
            break;
          case BTOR_OPT_OUTPUT_FORMAT_TAG_SMT2:
            opt = BTOR_OPT_OUTPUT_FORMAT;
            break;
          case BTOR_OPT_REWRITE_LEVEL_TAG_SMT2:
            opt = BTOR_OPT_REWRITE_LEVEL;
            break;
          ...
          case BTOR_OPT_BETA_REDUCE_ALL_TAG_SMT2:
            ...
            break;
          case BTOR_OPT_DUAL_PROP_TAG_SMT2:
            opt = BTOR_OPT_DUAL_PROP;
            break;
          case BTOR_OPT_JUST_TAG_SMT2:
            opt = BTOR_OPT_JUST;
            break;
#ifndef BTOR_DO_NOT_OPTIMIZE_UNCONSTRAINED
          case BTOR_OPT_UCOPT_TAG_SMT2:
            opt = BTOR_OPT_UCOPT;
            break;
#endif
          case BTOR_OPT_AUTO_CLEANUP_TAG_SMT2:
            opt = BTOR_OPT_AUTO_CLEANUP;
            break;
          case BTOR_OPT_PRETTY_PRINT_TAG_SMT2:
            opt = BTOR_OPT_PRETTY_PRINT;
            break;
          case BTOR_OPT_LOGLEVEL_TAG_SMT2:
            opt = BTOR_OPT_LOGLEVEL;
            break;
          case BTOR_OPT_PBRA_TAG_SMT2:
            opt = BTOR_OPT_PBRA;
            break;
...
btor_read_command_smt2 (BtorSMT2Parser * parser)
{
  ...
  if (parser->commands.model && tag == EOF)
    return !btor_perr_smt2 (
        parser, "expected ')' after 'model' at end-of-file");
  ...
  switch (tag)
    {
      case BTOR_SET_LOGIC_TAG_SMT2:
        ...
        if (tag == EOF)
          {
            parser->perrcoo = parser->lastcoo;
            return !btor_perr_smt2 (
                parser, "unexpected end-of-file after 'set-logic'");
          }
        if (tag == BTOR_INVALID_TAG_SMT2) { assert (parser->error); return 0; }
        if (!(tag & BTOR_LOGIC_TAG_CLASS_SMT2))
        ...
        if (tag == BTOR_QF_BV_TAG_SMT2) parser->res->logic = BTOR_LOGIC_QF_BV;
          ...
          parser->res->logic = BTOR_LOGIC_QF_AUFBV;
                                     ...
                                     parser->token.start);
        BTOR_MSG (boolector_get_btor_msg (parser->btor), 2, "logic %s", 
                  parser->token.start);
      ...
      case BTOR_CHECK_SAT_TAG_SMT2:
        ...
        if (parser->commands.check_sat++ && !parser->incremental)
          BTOR_MSG (boolector_get_btor_msg (parser->btor),
                    1,"WARNING additional 'check-sat' command");
        ...
        if ((width = boolector_get_width (parser->btor, exp)) != 1)
          {
            ...
            boolector_release (parser->btor, exp);
            return !btor_perr_smt2 (parser,
                "assert argument is a bit-vector of length %d", width);
          }
      ...
      case BTOR_GET_VALUE_TAG_SMT2:
        ...
        while (tag != EOF && tag != BTOR_RPAR_TAG_SMT2)
          {
            if (!btor_parse_term_smt2_aux (
                  parser, 1, tag, &exp, &coo, &tokens))
              {
                BTOR_RELEASE_STACK (parser->btor->mm, tokens);
                return 0;
              }
                        ...
                        BtorParseResult * res)
{
  ...
  if (!boolector_terminate (parser->btor))
    {
      if (!parser->commands.all)
        BTOR_MSG (boolector_get_btor_msg (parser->btor), 1, 
            "WARNING no commands in '%s'", parser->infile_name);
      else
        {
          if (!parser->commands.set_logic)
            BTOR_MSG (boolector_get_btor_msg (parser->btor), 1,
  ...
  if (delta < 0) delta = 0;
  BTOR_MSG (boolector_get_btor_msg (parser->btor), 1, 
      "parsed %d commands in %.2f seconds", parser->commands.all, delta);
