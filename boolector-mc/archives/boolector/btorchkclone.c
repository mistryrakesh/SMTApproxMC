...
btor_chkclone_mem (Btor * btor)
{
  ...
  assert (btor->mm->allocated 
          - (btor->msg->prefix 
             ? (strlen (btor->msg->prefix) + 1) * sizeof (char) : 0)
          == btor->clone->mm->allocated 
             - (btor->clone->msg->prefix 
                ? (strlen (btor->clone->msg->prefix) + 1) * sizeof (char) : 0));
...
#define BTOR_CHKCLONE_OPT(field)\
  do { \
    ...
    assert ((!btor->options.field.shrt && !clone->options.field.shrt) \
            || (btor->options.field.shrt && !strcmp ( \
                    clone->options.field.shrt, btor->options.field.shrt))); \
    assert (!strcmp (clone->options.field.lng, btor->options.field.lng)); \
    assert ((!btor->options.field.desc && !clone->options.field.desc) \
            || (btor->options.field.desc && !strcmp ( \
                    clone->options.field.desc, btor->options.field.desc))); \
...
btor_chkclone_opts (Btor * btor)
{
  ...
  assert ((!btor->options.auto_cleanup.shrt 
           && !clone->options.auto_cleanup.shrt)
  ...
  assert ((!btor->options.auto_cleanup.desc 
           && !clone->options.auto_cleanup.desc)
  ...
  if (real_aig != BTOR_AIG_FALSE)
    {
      ...
      for (i = 0; i < 2; i++)
        BTOR_CHKCLONE_AIGINV (children[i]);
...
#define BTOR_CHKCLONE_EXPID(exp, clone) \
  do { \
    assert (BTOR_REAL_ADDR_NODE (exp)->id \
            == BTOR_REAL_ADDR_NODE (clone)->id); \
    } while (0)
  ...
  do { \
    ...
    while (has_next_node_hash_table_iterator (&iter)) \
      { \
        ...
        BTOR_CHKCLONE_EXPID ( \
            next_node_hash_table_iterator (&iter), \
  ...
  if (real_exp->bits)
    {
      ...
      for (i = 0; i < len; i++)
        assert (real_exp->bits[i] == real_clone->bits[i]);
    }
  else
    {
          ...
          if (real_exp->arity)
          ...
          else
            {
              ...
              if (!BTOR_IS_FEQ_NODE (real_exp))
                ...
                {
                  assert (real_exp->vreads->exp1->id 
                          == real_clone->vreads->exp1->id);
                  assert (real_exp->vreads->exp2->id 
                          == real_clone->vreads->exp2->id);
                }
            }
        }
...
btor_chkclone_assignment_lists (Btor * btor)
{
  ...
  assert (btor->bv_assignments->count 
          == btor->clone->bv_assignments->count);
...
btor_chkclone_tables (Btor * btor)
{
      ...
      while (has_next_hash_table_iterator (&it))
        {
          assert (has_next_hash_table_iterator (&cit));
          assert (!strcmp (
              (char *) next_hash_table_iterator (&it),
  ...
  BTOR_CHKCLONE_NODE_PTR_HASH_TABLE (btor->var_rhs, 
                                     btor->clone->var_rhs);
  BTOR_CHKCLONE_NODE_PTR_HASH_TABLE (btor->fun_rhs, 
                                     btor->clone->fun_rhs);
          ...
          while (has_next_hash_table_iterator (&nit))
            {
              ...
              assert (!btor_compare_bv (
                        (BtorBitVector *) nit.bucket->data.asPtr,
...
btor_chkclone (Btor * btor)
{
  ...
  BTOR_CHKCLONE_NODE_ID_TABLE ( 
      btor->nodes_id_table, btor->clone->nodes_id_table);
  BTOR_CHKCLONE_NODE_UNIQUE_TABLE ( 
      btor->nodes_unique_table, btor->clone->nodes_unique_table); 
  BTOR_CHKCLONE_NODE_PTR_STACK ( 
      btor->functions_with_model, btor->clone->functions_with_model); 
