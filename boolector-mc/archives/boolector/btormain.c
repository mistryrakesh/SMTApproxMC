...
typedef struct BtorMainOpts
{
  ...
  /* rakesh - 2015-06-16 - added maxsolutions argument to generate multiple models */
  BtorMainOpt maxsolutions;
...
btormain_init_opts (BtorMainApp * app)
{
  ...
  BTORMAIN_INIT_OPT (app->opts.help, 1, "h", "help", 0, 0, 1,
            "print this message and exit");
  BTORMAIN_INIT_OPT (app->opts.copyright, 1, "c", "copyright", 0, 0, 1,
            "print copyright and exit");
  BTORMAIN_INIT_OPT (app->opts.version, 1, "V", "version", 0, 0, 1,
            "print version and exit");
  BTORMAIN_INIT_OPT (app->opts.time, 1, "t", "time", 0, 0, -1,
            "set time limit");
  BTORMAIN_INIT_OPT (app->opts.output, 1, "o", "output", 0, 0, 0, 
            "set output file for dumping");
  BTORMAIN_INIT_OPT (app->opts.smt2_model, 0, 0, "smt2_model", 0, 0, 1,
            "print model in SMT-LIB v2 format if model generation is enabled");
#ifdef BTOR_USE_LINGELING
  BTORMAIN_INIT_OPT (app->opts.lingeling, 0, 0, "lingeling", 0, 0, 1,
            "force Lingeling as SAT solver");
  BTORMAIN_INIT_OPT (app->opts.lingeling_opts, 0, 0, "lingeling_opts", 
            0, 0, 0, "set lingeling option(s) '--<opt>=<val>'");
  BTORMAIN_INIT_OPT (app->opts.lingeling_nofork, 0, 0, "lingeling_nofork", 
            0, 0, 0, "do not use 'fork/clone' for Lingeling");
#endif
#ifdef BTOR_USE_PICOSAT
  BTORMAIN_INIT_OPT (app->opts.picosat, 0, 0, "picosat", 0, 0, 1,
            "force PicoSAT as SAT solver");
#endif
#ifdef BTOR_USE_MINISAT
  BTORMAIN_INIT_OPT (app->opts.minisat, 0, 0, "minisat", 0, 0, 1,
           ...
           int print_dflt)
{
  ...
  assert (!strcmp (lng, "lingeling_opts")
          || (shrt && (2 * strlen (paramstr) + strlen(shrt) 
                       + strlen (lng) + 7 <= LEN_OPTSTR))
  ...
  for (i = 0; i < len; i++)
    lngstr[i] = lng[i] == '_' ? '-' : lng[i];
  lngstr[len] = '\0';
  sprintf (optstr, "  %s%s%s%s%s--%s%s%s", 
           shrt ? "-" : "",
           shrt ? shrt : "", 
...
"All of the following options can also be used in the form '-<short name>=<int>'\n" \
"and '--<long name>=<int>'. Flags are disabled with 0 and enabled with a pos.\n"\
"integer. Alternatively, use '-no-<short name>' and '--no-<long name>' for\n"\
"disabling, and '-<short name>' and '--<long name>' for enabling flags.\n" \
"Note that all of the following options can also be set via env. variables of\n" \
...
print_help (BtorMainApp * app)
{
  ...
  PRINT_MAIN_OPT (app, &to);
#endif
  to.shrt = "ds"; to.lng = "dump_smt2";
  to.desc = "dump formula in SMT-LIB v2 format";
  PRINT_MAIN_OPT (app, &to);
  to.shrt = "ds1"; to.lng = "dump_smt1";
  ...
  for (o = (char *) boolector_first_opt (app->btor); 
       o; 
       o = (char *) boolector_next_opt (app->btor, o))
    {
      if (!strcmp (o, BTOR_OPT_INPUT_FORMAT)
        ...
        continue;
      if (!strcmp (o, BTOR_OPT_INCREMENTAL) 
        ...
        fprintf (out, "\n");
      print_opt (app, o, boolector_get_opt_shrt (app->btor, o), 
          boolector_get_opt_dflt (app->btor, o), 
          boolector_get_opt_desc (app->btor, o), 1);
    }
...
print_static_stats (int sat_res)
{
  ...
  btormain_msg ("%s", sat_res == BOOLECTOR_SAT 
                      ? "sat"
...
{
  int l, k, d;
  l = strlen (str); k = strlen (suffix); d = l - k;
  if (d < 0) return 0;
...
boolector_main (int argc, char **argv)
{
  ...
  int maxSolutions = 0; // rakesh - 2015-06-18 - variable to store number of extra models to generate
  ...
  parse_res = BOOLECTOR_UNKNOWN; parse_status = BOOLECTOR_UNKNOWN;
  sat_res = BOOLECTOR_UNKNOWN;
  inc = 0; mgen = 0; pmodel = 0; dump = 0; force_sat = 0;
  ...
  for (i = 1; i < argc; i++)
    {
      arg = argv[i];
      len = strlen (argv[i]);
      isshrt = 0; isdisable = 0; isint = 0;
      readval = 0; val = 0; valstr = 0;


      /* rakesh - 2015-06-18 - handle parameter '--maxsolutions=<n>' */
      if (!strncmp(arg, "--maxsolutions=", strlen("--maxsolutions="))) {
          maxSolutions = atoi(arg + strlen("--maxsolutions="));
          assert(maxSolution >= 0);
          continue;
      }
      ...
      if (arg[0] != '-')
        {
          ...
          if (!btor_file_exists (g_app->infile_name))
                   ...
                   || has_suffix (g_app->infile_name, ".7z"))
            {
              ...
              if ((g_app->infile = popen (cmd, "r")))
                g_app->close_infile = 2;
      ...
      isshrt = arg[1] == '-' ? 0 : 1;
      j = isshrt ? 1 : 2;
      isdisable = len > 3 && arg[j]=='n' && arg[j+1]=='o' && arg[j+2]=='-';
...
ERR_UNEXPECTED_ARGUMENT:
              btormain_error (g_app,
                  "option '%s' does not expect an argument", errarg.start);
              goto DONE;
            }
...
ERR_INVALID_ARGUMENT:
              btormain_error (g_app, "invalid argument for '%s', expected int",
                              errarg.start);
              goto DONE;
            }
...
SET_OUTPUT_FORMAT:
      ...
      /* >> btor options */
        ...
        {
          for (o = (char *) boolector_first_opt (g_app->btor);
               o;
          ...
          if (IS_BTOR_OPT ("i", BTOR_OPT_INCREMENTAL))
            {
              ...
              inc = isdisable || (readval && isint && val == 0)
                    ? 0 : inc | BTOR_PARSE_MODE_BASIC_INCREMENTAL;
          ...
          else if (IS_BTOR_OPT ("I", BTOR_OPT_INCREMENTAL_ALL))
            {
              ...
              if (isdisable || (readval && isint && val == 0))
                ...
                {
                  boolector_set_opt (g_app->btor, o, 
          ...
          /* options that can be used as a flag or with an argument */
            ...
            {
              if (isdisable && HAS_UNEXPECTED_ARGUMENT)
                goto ERR_UNEXPECTED_ARGUMENT;
              ...
              if ((IS_BTOR_OPT ("dp", BTOR_OPT_DUAL_PROP)
                   ...
                   && boolector_get_opt_val (g_app->btor, BTOR_OPT_DUAL_PROP)))
                {
                  btormain_error (g_app,
                      ...
                      BTOR_OPT_DUAL_PROP, BTOR_OPT_JUST);
                  goto DONE;
                }
              ...
              if (isdisable || (readval && isint && val == 0))
                ...
                boolector_set_opt (g_app->btor, o,
                    boolector_get_opt_val (g_app->btor, o) + 1);
            }
        }
    }
  ...
  mgen = !mgen && g_app->opts.smt2_model.val
           ? g_app->opts.smt2_model.val : mgen;

  // TODO: disabling model generation not yet supported (ma)
  if (mgen > 0)
    boolector_set_opt (g_app->btor, BTOR_OPT_MODEL_GEN, mgen);
  ...
  if (g_app->opts.lingeling.val)
    {
      ...
      if (!boolector_set_sat_solver_lingeling (g_app->btor,
            lglopts, g_app->opts.lingeling_nofork.val))
  ...
  if (g_app->opts.lingeling_nofork.val && !g_app->opts.lingeling.val)
    {
      btormain_error (g_app, "option '%s' is invalid if Lingeling is not "
          "configured as SAT solver", errarg.start);
      goto DONE;
    }
#endif
      ...
      switch (val)
        {
          case BTOR_INPUT_FORMAT_BTOR:
            ...
            parse_res = boolector_parse_btor (
                g_app->btor, g_app->infile, g_app->infile_name, g_app->outfile,
                &parse_err_msg, &parse_status);
            break;
          case BTOR_INPUT_FORMAT_SMT1:
            if (g_verbosity)
              btormain_msg ("SMT-LIB v1 input forced through cmd line options");
            parse_res = boolector_parse_smt1 (
                g_app->btor, g_app->infile, g_app->infile_name, g_app->outfile,
                &parse_err_msg, &parse_status);
            break;
          case BTOR_INPUT_FORMAT_SMT2: 
            ...
            parse_res = boolector_parse_smt2 (
                g_app->btor, g_app->infile, g_app->infile_name, g_app->outfile,
                &parse_err_msg, &parse_status);
            break;
        }
      ...
      if (pmodel && sat_res == BOOLECTOR_SAT)
        {
          ...
          // boolector_print_model (g_app->btor,
              // g_app->opts.smt2_model.val ? "smt2" : "btor", g_app->outfile);


            /* rakesh - 2015-06-18 - add constraints in loop */

            int i;

            for (i = 1; i <= maxSolutions; ++i) {
                add_model_as_constraint(g_app->btor);

                int new_sat_res = boolector_sat(g_app->btor);
                if (new_sat_res == BOOLECTOR_SAT) {
                    // printf("\nprinting model: %d\n", i+2);
                    printf("sat\n");
                    // boolector_print_model(g_app->btor,
                            // g_app->opts.smt2_model.val ? "smt2" : "btor", g_app->outfile);
                } else if (new_sat_res == BOOLECTOR_UNSAT) {
                    printf("unsat\n");
                    break;
                } else if (new_sat_res == BOOLECTOR_UNKNOWN) {
                    printf("unknown\n");
                    break;
                } else {
                    printf("error\n");
  ...
  if (sat_res == BOOLECTOR_SAT && parse_status == BOOLECTOR_UNSAT)
    btormain_error (g_app, "'sat' but status of benchmark in '%s' is 'unsat'",
                    g_app->infile_name);
  else if (sat_res == BOOLECTOR_UNSAT && parse_status == BOOLECTOR_SAT)
    btormain_error (g_app, "'unsat' but status of benchmark in '%s' is 'sat'",
                    g_app->infile_name);
...
DONE:
  if (g_app->done) res = BTOR_SUCC_EXIT;
  else if (g_app->err) res = BTOR_ERR_EXIT;
  else if (sat_res == BOOLECTOR_UNSAT) res = BTOR_UNSAT_EXIT;
  else if (sat_res == BOOLECTOR_SAT) res = BTOR_SAT_EXIT;

  assert (res == BTOR_ERR_EXIT
          || res == BTOR_SUCC_EXIT
          || res == BTOR_SAT_EXIT
          || res == BTOR_UNSAT_EXIT
          || res == BTOR_UNKNOWN_EXIT);

  if (g_app->close_infile == 1) fclose (g_app->infile);
  else if (g_app->close_infile == 2) pclose (g_app->infile);
