...
static BtorNode *
clone_exp (Btor * clone, BtorNode * exp, 
...
//      assert (!lambda->head || !BTOR_IS_INVALID_NODE (lambda->head));
      assert (!lambda->body || !BTOR_IS_INVALID_NODE (
                                    BTOR_REAL_ADDR_NODE (lambda->body)));
      
//      BTOR_PUSH_STACK_IF (((BtorLambdaNode *) exp)->head, mm, *nodes, 
//                          (BtorNode **) &((BtorLambdaNode *) res)->head);
      BTOR_PUSH_STACK_IF (((BtorLambdaNode *) exp)->body, mm, *nodes,
  ...
  while (!BTOR_EMPTY_STACK (rhos))
    {
      ...
      *chtable = btor_clone_ptr_hash_table (mm, *htable,
        btor_clone_key_as_node, btor_clone_data_as_node_ptr, exp_map, exp_map);
    }
...
#define CLONE_PTR_HASH_TABLE_DATA(table, data_func) \
  do { \
    ...
    BTORLOG ("  clone "#table" table: %.3f s", (btor_time_stamp () - delta)); \
    CHKCLONE_MEM_PTR_HASH_TABLE(table); \
  } while (0)
                ...
                int exp_layer_only)
{
  ...
  if (BTOR_SIZE_STACK (btor->stats.lemmas_size) > 0)
    {
      BTOR_CNEWN (mm, clone->stats.lemmas_size.start,
                  BTOR_SIZE_STACK (btor->stats.lemmas_size));
      clone->stats.lemmas_size.end =
        clone->stats.lemmas_size.start +
        BTOR_SIZE_STACK (btor->stats.lemmas_size);
      clone->stats.lemmas_size.top =
        clone->stats.lemmas_size.start +
        BTOR_COUNT_STACK (btor->stats.lemmas_size);
      memcpy (clone->stats.lemmas_size.start, btor->stats.lemmas_size.start,
  ...
  assert ((allocated += BTOR_SIZE_STACK (btor->stats.lemmas_size) 
                        * sizeof (int)) 
            == clone->mm->allocated);
  ...
  if (exp_layer_only)
  ...
  else
    {
      ...
      BTORLOG ("  clone array assignments: %.3f s", 
  ...
  for (i = 1; i < BTOR_COUNT_STACK (btor->nodes_id_table); i++)
    {
      ...
      allocated += cur->bytes;
      if (cur->bits)
        allocated += strlen (cur->bits) + 1;
      if (cur->invbits)
        allocated += strlen (cur->invbits) + 1;
  ...
  while (has_next_hash_table_iterator (&it))
    str_bytes += (strlen ((char *) next_hash_table_iterator (&it)) + 1)
                 * sizeof (char);
  ...
  if (btor->score)
    {
      ...
      if (h == BTOR_JUST_HEUR_BRANCH_MIN_APP)
        {
          clone->score = btor_clone_ptr_hash_table (mm, btor->score, 
            btor_clone_key_as_node, btor_clone_data_as_htable_ptr, emap, emap); 
      ...
      while (!BTOR_EMPTY_STACK (stack))
        {
          ...
          if (!btor_find_in_ptr_hash_table (
                   clone->synthesized_constraints, exp))
  ...
  sprintf (prefix, "[%s]%s%s", clone_prefix, 
           btor->msg->prefix ? " " : "",
...
{
  assert (btor);
  return clone_aux_btor (btor, 0, 0,
              !btor_has_clone_support_sat_mgr (btor_get_sat_mgr_btor (btor)));
}

Btor *
btor_clone_exp_layer (Btor * btor, 
                      BtorNodeMap ** exp_map, 
                      BtorAIGMap ** aig_map)
{
  assert (btor);
