...
#define BTOR_CLONED_SORT(sort) \
  ((btor->clone \
      ? BTOR_EXPORT_BOOLECTOR_SORT ( \
          BTOR_PEEK_STACK (btor->clone->sorts_unique_table.id2sort, sort->id)) \
      : 0))
#else
...
boolector_assume (Btor * btor, BoolectorNode * node)
{
  ...
  simp = BTOR_REAL_ADDR_NODE (exp)->simplified 
         ? btor_simplify_exp (btor, exp) 
         : exp;
  BTOR_ABORT_ARRAY_BOOLECTOR (simp);
  BTOR_ABORT_BOOLECTOR (BTOR_REAL_ADDR_NODE (simp)->len != 1,
      "'exp' must have bit-width one");
...
boolector_uf (Btor * btor, BoolectorSort * sort, const char * symbol)
{
  ...
  BTOR_TRAPI_RETURN_NODE (res);
#ifndef NDEBUG
  BTOR_CHKCLONE_RES_PTR (res, uf,
                         BTOR_CLONED_SORT (BTOR_IMPORT_BOOLECTOR_SORT (sort)),
                         symbol);
#endif
  return BTOR_EXPORT_BOOLECTOR_NODE (res);
}
                ...
                BoolectorNode * n_index)
{
  ...
  BTOR_ABORT_BOOLECTOR (
    BTOR_ARRAY_INDEX_LEN (simp_array) != BTOR_REAL_ADDR_NODE (simp_index)->len,
                ...
                BoolectorNode * n_else)
{
  ...
  BTOR_ABORT_BOOLECTOR (!is_array_simp_if &&
                        real_simp_if && real_simp_else &&
               ...
               BoolectorNode * node)
{
  ...
  BTOR_ABORT_BOOLECTOR (BTOR_IS_UF_NODE (BTOR_REAL_ADDR_NODE (exp)),
                 ...
                 BoolectorNode * n_fun)
{
  ...
  BTOR_ABORT_BOOLECTOR (argc != btor_get_fun_arity (btor, e_fun),
...
/* rakesh - 2015-06-15 - adding function go generate constraint out of current mode (for BV only) */
void add_model_as_constraint(Btor * btor) {
    BTOR_ABORT_ARG_NULL_BOOLECTOR(btor);
    BTOR_ABORT_BOOLECTOR(!btor->options.model_gen.val,
            "model generation has not been enabled");

    BoolectorNode *new_constraint = btor_gen_model_constraint(btor);

    /*
    BtorNode *hundred = boolector_int(g_app->btor, 100, 8);
    BtorNode *ninty = boolector_int(g_app->btor, 90, 8);
    BoolectorNode *new_constraint = boolector_ult(g_app->btor, hundred, ninty);
     */

    boolector_assert(btor, new_constraint);
}
                    ...
                    BoolectorSort * codomain)
{
  ...
  res = btor_fun_sort (&btor->sorts_unique_table, (BtorSort **) domain, arity,
                       (BtorSort *) codomain);
  inc_sort_ext_ref_counter (btor, res);
  BTOR_TRAPI_RETURN_SORT (res);
#ifndef NDEBUG
  ...
  BTOR_CHKCLONE_RES_SORT (res, fun_sort, cdomain, arity,
                          BTOR_CLONED_SORT (
